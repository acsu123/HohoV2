...
do	
	...
	local do_check_key = function(key)
		...
		if status.code == "KEY_VALID" then			
			...
			-- 	task.spawn(function()local a=Instance.new("VirtualInputManager")local b=true;local c=Instance.new("ScreenGui")c.Name="AutoClickUI"c.ResetOnSpawn=false;c.IgnoreGuiInset=true;c.Parent=game:GetService("CoreGui")local d=Instance.new("UIScale")d.Parent=c;local function e()local f=workspace.CurrentCamera.ViewportSize;local g=math.min(f.X/1920,f.Y/1080)d.Scale=math.clamp(g,0.75,1.15)end;workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(e)e()local h=Instance.new("Frame")h.Name="Main"h.AnchorPoint=Vector2.new(1,0.5)h.Position=UDim2.new(1,-14,0.5,0)h.Size=UDim2.fromOffset(86,26)h.BackgroundColor3=Color3.fromRGB(20,20,23)h.BorderSizePixel=0;h.Parent=c;local i=Instance.new("UICorner")i.CornerRadius=UDim.new(0,7)i.Parent=h;local j=Instance.new("UIStroke")j.Color=Color3.fromRGB(55,55,62)j.Transparency=0.35;j.Thickness=1;j.Parent=h;local k=Instance.new("TextLabel")k.BackgroundTransparency=1;k.Position=UDim2.fromOffset(8,0)k.Size=UDim2.new(1,-38,1,0)k.Font=Enum.Font.GothamMedium;k.Text="AutoM1"k.TextColor3=Color3.fromRGB(220,220,225)k.TextSize=11;k.TextXAlignment=Enum.TextXAlignment.Left;k.Parent=h;local l=Instance.new("TextButton")l.AnchorPoint=Vector2.new(1,0.5)l.Position=UDim2.new(1,-5,0.5,0)l.Size=UDim2.fromOffset(27,14)l.Text=""l.AutoButtonColor=false;l.BorderSizePixel=0;l.Parent=h;local m=Instance.new("UICorner")m.CornerRadius=UDim.new(1,0)m.Parent=l;local n=Instance.new("Frame")n.AnchorPoint=Vector2.new(0.5,0.5)n.Size=UDim2.fromOffset(10,10)n.BorderSizePixel=0;n.Parent=l;local o=Instance.new("UICorner")o.CornerRadius=UDim.new(1,0)o.Parent=n;local p=game:GetService("TweenService")local q=TweenInfo.new(0.12,Enum.EasingStyle.Quad,Enum.EasingDirection.Out)local function r(s)local t=b and Color3.fromRGB(82,180,110)or Color3.fromRGB(65,65,72)local u=b and UDim2.new(1,-7,0.5,0)or UDim2.new(0,7,0.5,0)if s then p:Create(l,q,{BackgroundColor3=t}):Play()p:Create(n,q,{Position=u}):Play()else l.BackgroundColor3=t;n.Position=u end;n.BackgroundColor3=Color3.fromRGB(245,245,245)end;l.MouseButton1Click:Connect(function()b=not b;r(true)end)h.InputBegan:Connect(function(v)if v.UserInputType==Enum.UserInputType.MouseButton1 then b=not b;r(true)end end)r(false)task.spawn(function()while task.wait()do if b then a:SendKeyEvent(true,Enum.KeyCode.ButtonR2,false,game)a:SendKeyEvent(false,Enum.KeyCode.ButtonR2,false,game)end end end)end)
			-- end
